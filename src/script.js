...
let foodRepository = (function () {
  ...
  function spinnerRun() {
    ...
    setInterval(() => {
      spinner.innerHTML = foodSpinner[Math.floor(Math.random() * foodSpinner.length)];
    }, 100);
  }
    ...
    return fetch(apiUrl)
      ...
      .then(function (json) {
        json.meals.forEach(function (item) {
          ...
          food.ingredients = new Array(20)
            .fill(1)
            .map((element, i) => item['strIngredient' + (element + i)])
            .filter((food) => !!food);
          add(food);
        });
      ...
      return fetch(searchApiUrl)
        ...
        .then(function (json) {
          json.meals.forEach(function (item) {
            ...
            searchFood.ingredients = new Array(20)
              .fill(1)
              .map((element, i) => item['strIngredient' + (element + i)])
              .filter((food) => !!food);
